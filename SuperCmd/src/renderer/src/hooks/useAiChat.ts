...
export function useAiChat({ onExitAiMode, setAiMode }: UseAiChatOptions): UseAiChatReturn {
  ...
  const startAiChat = useCallback(
    (searchQuery: string) => {
      ...
      const sessionId = `supercmd-ai-${Date.now()}`;
      ...
      setAiStreaming(true);
      setAiMode(true);
      window.electron.aiAsk(requestId, searchQuery, { sessionId });
    },
  ...
  const submitAiQuery = useCallback(
    (query: string) => {
      ...
      const requestId = `ai-${Date.now()}`;
      const sessionId = aiSessionIdRef.current || `supercmd-ai-${Date.now()}`;
      aiRequestIdRef.current = requestId;
      aiSessionIdRef.current = sessionId;
      aiStreamingRef.current = true;
      setAiQuery(query);
      setAiResponse('');
      setAiStreaming(true);
      window.electron.aiAsk(requestId, query, { sessionId });
    },
    [],
  );
