...
*   **开发机（当前这台电脑）：** 只负责修改 Workspace、编写文档、改造 SuperCmd、做最小本地 smoke test。
...
### 阶段二：前端换脑与“机甲合体” (The Frontend: SuperCmd)
*目标：在保证大脑正常运转后，为其打造专属的 Mac 极简聚光灯交互外壳。*
4.  **跑通 SuperCmd 极简原型：**
    *   在本地 `SuperCmd` 源码目录下安装 Node 依赖包并启动开发环境。
    *   体验原生的全局唤醒、渲染质感与 Markdown 组件，熟悉其 UI 交互流。
5.  **外科手术式前端“换核”：** 
    *   定位并切断 SuperCmd 默认调用的远端大模型 API。
    *   将输入框的发送请求拦截，改为通过本地端口直连 阶段一 中跑通的 OpenClaw 服务。
    *   新增定制指令（如 `Vibe Dump`、`Ask Brain`），实现“一键倾倒后隐藏”的标志性 0 阻力体验。
