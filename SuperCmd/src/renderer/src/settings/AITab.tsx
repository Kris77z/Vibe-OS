...
const PROVIDER_OPTIONS = [
  ...
  { id: 'openclaw' as const, label: 'OpenClaw', description: 'Route requests through your Vibe-OS brain' },
];
  ...
  const genericModels = ai.provider === 'ollama' && ollamaRunning
    ...
    : ai.provider === 'openai-compatible' && ai.openaiCompatibleModel
      ...
      : ai.provider === 'openclaw'
        ? [{
            id: `openclaw:${String(ai.openclawAgentId || '').trim() || 'main'}`,
            label: `OpenClaw (${String(ai.openclawAgentId || '').trim() || 'main'})`,
          }]
            ...
            <div className={`px-4 py-3.5 md:px-5 space-y-3 self-start ${activeTab === 'llm' ? '' : 'hidden'}`}>
              ...
              <div>
                    ...
                    {PROVIDER_OPTIONS.map((p) => (
                      <button
                        ...
                        onClick={() => {
                          ...
                          if (p.id === 'openclaw') {
                            const nextAgentId = String(ai.openclawAgentId || '').trim() || 'main';
                            updateAI({ provider: p.id, defaultModel: `openclaw:${nextAgentId}` });
                            return;
                          }
                ...
                {ai.provider === 'openclaw' && (
                  <div className="space-y-3">
                    <div>
                      <label className="text-[0.75rem] text-[var(--text-muted)] mb-1 block">Gateway Base URL</label>
                      <input
                        type="text"
                        value={ai.openclawBaseUrl}
                        onChange={(e) => updateAI({ openclawBaseUrl: e.target.value.trim() })}
                        placeholder="http://127.0.0.1:18789"
                        className="w-full bg-[var(--ui-segment-bg)] border border-[var(--ui-divider)] rounded-md px-2.5 py-2 text-sm text-[var(--text-secondary)] placeholder:text-[color:var(--text-subtle)] focus:outline-none focus:border-blue-500/50"
                      />
                      <p className="text-[0.625rem] text-[var(--text-subtle)] mt-1">SuperCmd will call <code>/v1/responses</code> on this gateway.</p>
                    </div>

                    <div>
                      <label className="text-[0.75rem] text-[var(--text-muted)] mb-1 block">Gateway Token</label>
                      <div className="relative">
                        <input
                          type={showOpenClawToken ? 'text' : 'password'}
                          value={ai.openclawGatewayToken}
                          onChange={(e) => updateAI({ openclawGatewayToken: e.target.value.trim() })}
                    ...
                    <div>
                      <label className="text-[0.75rem] text-[var(--text-muted)] mb-1 block">Agent ID</label>
                      <input
                        type="text"
                        value={ai.openclawAgentId}
                        onChange={(e) => {
                          const agentId = e.target.value.trim() || 'main';
                          updateAI({
                            openclawAgentId: agentId,
                            defaultModel: `openclaw:${agentId}`,
                          });
                        }}
                        placeholder="main"
                        className="w-full bg-[var(--ui-segment-bg)] border border-[var(--ui-divider)] rounded-md px-2.5 py-2 text-sm text-[var(--text-secondary)] placeholder:text-[color:var(--text-subtle)] focus:outline-none focus:border-blue-500/50"
                      />
                      <p className="text-[0.625rem] text-[var(--text-subtle)] mt-1">Use the OpenClaw agent id you want this desktop shell to talk to.</p>
                    </div>
                  </div>
                )}
