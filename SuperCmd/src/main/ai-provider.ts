...
interface ModelRoute {
  provider: 'openai' | 'anthropic' | 'gemini' | 'ollama' | 'openai-compatible' | 'openclaw';
  modelId: string;
}
...
function resolveModel(model: string | undefined, config: AISettings): ModelRoute {
  ...
  if (model) {
    if (model === 'openclaw') {
      return {
        provider: 'openclaw',
        modelId: String(config.openclawAgentId || '').trim() || 'main',
      };
    }
    if (model.startsWith('openclaw:')) {
      return {
        provider: 'openclaw',
        modelId: model.slice('openclaw:'.length) || 'main',
      };
    }
    ...
    for (const prefix of prefixes) {
      if (model.startsWith(prefix)) {
        return {
          provider: prefix.slice(0, -1) as 'openai' | 'anthropic' | 'gemini' | 'ollama' | 'openai-compatible',
          modelId: model.slice(prefix.length),
        };
      }
    }
  ...
  if (config.defaultModel) {
    if (config.defaultModel === 'openclaw') {
      return {
        provider: 'openclaw',
        modelId: String(config.openclawAgentId || '').trim() || 'main',
      };
    }
    if (config.defaultModel.startsWith('openclaw:')) {
      return {
        provider: 'openclaw',
        modelId: config.defaultModel.slice('openclaw:'.length) || 'main',
      };
    }
  ...
  const defaults: Record<string, string> = {
    ...
    openclaw: config.openclawAgentId?.trim() || 'main',
  };
...
function hasProviderCredentials(provider: ModelRoute['provider'], config: AISettings): boolean {
  switch (provider) {
    ...
    case 'openclaw':
      return !!String(config.openclawBaseUrl || '').trim();
    default:
      return false;
  }
}
...
function resolveOpenClawResponsesUrl(baseUrl: string): URL {
  const raw = String(baseUrl || '').trim() || 'http://127.0.0.1:18789';
...
function extractOpenClawResponseText(response: any): string {
...
async function* streamOpenClaw(
  ...
  const url = resolveOpenClawResponsesUrl(baseUrl);
  const normalizedAgentId = String(agentId || '').trim() || 'main';
  const body: Record<string, unknown> = {
    model: `openclaw:${normalizedAgentId}`,
    input: prompt,
    stream: true,
  };
  if (systemPrompt) {
  ...
  if (String(gatewayToken || '').trim()) {
    headers.Authorization = `Bearer ${String(gatewayToken).trim()}`;
  }
    ...
    for (const line of lines) {
      ...
      switch (parsed?.type) {
        ...
        case 'response.output_text.done':
          if (!sawDelta && typeof parsed.text === 'string' && parsed.text) {
            emittedFallbackText = true;
            yield parsed.text;
          }
          break;
        case 'response.failed':
          throw new Error(parsed?.response?.error?.message || 'OpenClaw request failed');
        case 'response.completed':
          if (!sawDelta && !emittedFallbackText) {
            const finalText = extractOpenClawResponseText(parsed?.response);
            if (finalText) {
              emittedFallbackText = true;
              yield finalText;
            }
