...
`;
  ...
  ipcMain.handle(
    ...
    async (
      ...
      requestId: string,
      prompt: string,
      options?: { model?: string; creativity?: number; systemPrompt?: string; sessionId?: string }
    ) => {
      const s = loadSettings();
      ...
      try {
        // OpenClaw already injects workspace context and memory; avoid duplicating it here.
        const memoryContextSystemPrompt = s.ai?.provider === 'openclaw'
          ? ''
          : await buildMemoryContextSystemPrompt(
              s,
              String(prompt || ''),
              { limit: 6 }
            );
